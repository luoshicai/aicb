...
from utils.bench import bench_kineto
...
# vLLM 项目中 RMSNorm（Root Mean Square Layer Normalization）操作的封装，支持可选的 残差连接（residual connection）融合。
...
def rmsnorm_vllm(
    ...
    if isinstance(output, tuple):
        # 如果是融合模式（返回 tuple），则将 x 和 residual 都恢复为原始形状（如 [B, S, D]）；
        output = (output[0].view(orig_shape), output[1].view(orig_shape))
    else:
        # 如果是普通模式（返回单个 tensor），则只恢复 out 的形状。
...
class Qwen3MoeAttention(torch.nn.Module):
    ...
    # 该函数目的不是执行实际的模型前向计算，而是对一个 RMSNorm（Root Mean Square Layer Normalization）操作进行性能基准测试（benchmarking）
    def _norm(self):
        # 获取配置参数
        ...
        # 执行性能基准测试
        # 对给定函数 fn 中调用的 CUDA kernel（通过名称指定）进行高精度、抗干扰的 GPU 执行时间测量，并返回平均单次耗时（秒）。
        t = bench_kineto(test_func, 'vllm::fused_ad', suppress_kineto_output=True)
        return t

    # 测量一次 FP8 量化算子 per_token_group_quant_fp8() 的执行时间，用于评估注意力模块的量化代价。
...
class Qwen3MoeSparseMoeBlock(torch.nn.Module):
    ...
    # 用分组（grouped）FP8 GEMM kernel 做一次基准测试（benchmark），并返回测得的时间 t。
    ...
    def forward(self):
        # 计算每个 token 的 gating 分数（router logits）。
